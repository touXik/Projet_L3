...
if (isset($_POST['submit'])){
    extract($_POST);
     ...
     if (!empty($lastname) && !empty($firstname) && !empty($email) && !empty($password) && !empty($cpassword)){
        ...
        $c = $bdd->prepare("SELECT email FROM user WHERE email = :email");
        ...
        if($result ==0){
             ...
             $q = $bdd->prepare("INSERT INTO user(lastname,firstname,email,password) VALUES(:lastname,:firstname,:email,:password)");
        $q->execute([
            'lastname'=>$lastname,
            'firstname'=>$firstname,
            'email'=>$email,
           ...
           include '../html/alrt/alrt2.html';

        } else{
            
             include '../html/register.html';
             include '../html/alrt/alrt.html';
